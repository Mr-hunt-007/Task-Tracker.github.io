...
import type { Task, List, Tag, View, Filter, SortOption, ViewLayout } from "@/types"
...
export default function TaskTracker() {
  ...
  useEffect(() => {
    ...
    const savedLayout = localStorage.getItem("taskTrackerViewLayout")
    ...
    try {
      ...
      if (savedLayout) setViewLayout(JSON.parse(savedLayout))
    } catch (error) {
    ...
    // If no tasks were loaded, initialize with mock data
  ...
  useEffect(() => {
    ...
    localStorage.setItem("taskTrackerViewLayout", JSON.stringify(viewLayout))
  }, [tasks, lists, tags, completedPomodoros, currentView, viewLayout])
  ...
  // Handle drag end for kanban board and sortable lists
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      ...
      // If reordering within a column or list
      setTasks((tasks) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-950/50 transition-colors duration-200">
          {renderCurrentView()}
        </main>
      </div>
